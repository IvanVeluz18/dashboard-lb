...
$rs = DBGetData2("SELECT 
 ...
 WHERE so.created_at BETWEEN '$Date1' AND '$Date2 23:59:59' AND so.status='PAID' AND ss.service_id IS NOT NULL
 GROUP BY month, branch, service, gender");
DBClose();
...
foreach($rs as $r){
 ...
 $svc[$r['Service']]['Amount'] += $r['Amount'];
 $svc[$r['Service']]['Qty'] += $r['Qty'];
